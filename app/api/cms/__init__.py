...
def create_cms() -> APIRouter:
    """
    自动扫描 app.api.v1 下所有包含 rp 对象的模块，并注册到 router_v1
    """
    router_cms = APIRouter(prefix="/cms", tags=["cms"])

    package_name = "app.api.cms"
    package = importlib.import_module(package_name)

    print("🚀 正在扫描并注册 cms 模块 ...\n")
    ...
    for _, module_name, is_pkg in pkgutil.iter_modules(package.__path__):
        ...
        try:
            ...
            if rp:
                router_cms.include_router(rp)
                route_count = len(rp.routes)
                print(
                    f"✅ 已注册子模块: {module_name:<12} | prefix={rp.prefix or '/'} | tags={rp.tags} | routes={route_count}")
            else:
